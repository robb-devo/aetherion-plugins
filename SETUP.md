...
QA Mineflayer bots (mine / forage / catch / roam / combat / fish / farm / trade / quest / pad) live in **AetherionStressBots**. They only **start from Dev menu / commands** when `plugins/AetherionStressBots/config.yml` has `testbots.enabled: true`. Tonight’s 2h mix: [docs/bots/overnight-ready.md](docs/bots/overnight-ready.md).
