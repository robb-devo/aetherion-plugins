 ...
 * Same {@link Quest} objects the live quest manager already runs — story quests stay in
 * {@link de.aetherion.quests.quest.QuestRegistry}.
 */
public final class EditorQuestStorage {
        ...
        if (!file.exists()) {
            try {
                ...
                empty.options().header("""
                        Quests created from the NPC / Quest Editor.
    ...
    public synchronized Quest createAndSave(String title, String npcId, QuestManager manager) {
        Set<String> taken = new HashSet<>(quests.keySet());
        if (manager != null) {
    ...
    static Quest read(String id, ConfigurationSection section) {
        ...
        quest.setServiceId(section.getString("service", EditorQuestFactory.SERVICE));
